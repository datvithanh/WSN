...
import numpy as np
from mfea.fitness import get_fitness
from utils.load_input import WsnInput
from mfea.constructor import HopConstructor, LayerContructor
...
def run_ga(inp: WsnInput, logger=None):
    ...
    logger.info("Start!")
    hopConstructor = HopConstructor(inp.num_of_sensors, inp.num_of_relays, inp.num_of_relay_positions, inp.all_vertex)
    layerContructor = LayerContructor()
    ...
    toolbox.register("evaluate", get_fitness, max_hop=inp.max_hop, constructor=hopConstructor)
    ...
    tmp = hopConstructor.gen_graph(best_ind)
    return best_ind
